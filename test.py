...
class TestPortfolioOptimization(unittest.TestCase):
    ...
    def test_dividend_optimization_request_validation(self):
        ...
        from dividend_optimizer import DividendOptRequest
        from data import StockItem, ExchangeEnum
        
        # Valid request with default parameters
        valid_request = DividendOptRequest(
            stocks=[
        ...
        self.assertEqual(valid_request.entropy_weight, 0.05)  # Default
        self.assertEqual(valid_request.price_lookback_days, 756)  # Default ~3 years
        
        # Test with custom parameters
        custom_request = DividendOptRequest(
            stocks=[
                StockItem(ticker="ITC", exchange=ExchangeEnum.NSE),
                StockItem(ticker="HDFCBANK", exchange=ExchangeEnum.NSE),
                StockItem(ticker="TCS", exchange=ExchangeEnum.NSE)
            ],
            entropy_weight=0.1,
            vol_cap=0.25,
            use_median_ttm=True
        )
        self.assertEqual(custom_request.entropy_weight, 0.1)
        self.assertEqual(custom_request.vol_cap, 0.25)
        self.assertTrue(custom_request.use_median_ttm)
        
        print("✅ Dividend optimization request validation test passed")

    def test_dividend_optimization_function(self):
        """Test dividend optimization function with mocked data"""
        from dividend_optimizer import optimize_dividend_portfolio, DividendOptRequest
        from data import StockItem, ExchangeEnum, APIError, ErrorCode
        
        # Test insufficient stocks error
        with self.assertRaises(APIError) as context:
            req = DividendOptRequest(
                stocks=[StockItem(ticker="ITC", exchange=ExchangeEnum.NSE)]  # Only 1 stock
            )
            optimize_dividend_portfolio(req)
        self.assertEqual(context.exception.code, ErrorCode.INSUFFICIENT_STOCKS)
        
        # Test ticker formatting
        from dividend_optimizer import _format_tickers
        
        # NSE tickers
        stocks_nse = [StockItem(ticker="ITC", exchange=ExchangeEnum.NSE)]
        self.assertEqual(_format_tickers(stocks_nse), ["ITC.NS"])
        
        # BSE tickers
        stocks_bse = [StockItem(ticker="TCS", exchange=ExchangeEnum.BSE)]
        self.assertEqual(_format_tickers(stocks_bse), ["TCS.BO"])
        
        # Mixed exchanges
        stocks_mixed = [
            StockItem(ticker="ITC", exchange=ExchangeEnum.NSE),
            StockItem(ticker="TCS", exchange=ExchangeEnum.BSE)
        ]
        self.assertEqual(_format_tickers(stocks_mixed), ["ITC.NS", "TCS.BO"])
    ...
    def test_dividend_optimization_endpoint(self):
        """Test dividend optimization endpoint"""
        from fastapi.testclient import TestClient
        from srv import app
        
        client = TestClient(app)
        
        # Test with invalid request (insufficient stocks)
        request_data = {
            "stocks": [
        ...
        response = client.post("/dividend-optimize", json=request_data)
        self.assertEqual(response.status_code, 422)  # Should fail validation
        
        print("✅ Dividend optimization endpoint test passed")
    ...
    def test_dividend_response_model_structure(self):
        ...
        # Validate structure
        self.assertEqual(len(response.weights), 2)
        self.assertEqual(response.portfolio_yield, 0.035)
    ...
    def test_entropy_yield_optimizer_initialization(self):
        """Test entropy yield optimizer initialization"""
        from divopt import EntropyYieldOptimizer, EntropyYieldConfig
        
        # Test with minimum required tickers
        tickers = ["AAPL", "MSFT"]
        optimizer = EntropyYieldOptimizer(tickers)
        self.assertEqual(optimizer.tickers, tickers)
        ...
        optimizer2 = EntropyYieldOptimizer(tickers, config)
        self.assertEqual(optimizer2.cfg.entropy_weight, 0.1)
        self.assertEqual(optimizer2.cfg.vol_cap, 0.3)
        self.assertTrue(optimizer2.cfg.use_median_ttm)
        
        # Test error with insufficient tickers
        with self.assertRaises(ValueError) as context:
            EntropyYieldOptimizer(["AAPL"])  # Only 1 ticker
